...
//								(int) (from.getBlue() * .15));
	...
	public void hideEasyStego(int[] words) {
		Pixel[][] pixels = this.getPixels2D();
		for (int row = 0, hold = 0; row < pixels.length && hold < words.length; row++) {
			for (int col = 0; col < pixels[0].length && hold < words.length; col++) {
	...
	public ArrayList<String> getEasyNums() {
		...
		for (int row = 0; row < pixels.length; row++) {
			for (int col = 0; col < pixels[0].length; col++) {
				if (pixels[row][col].getBlue() < 100 && pixels[row][col].getGreen() < 100) {
					temp = "" + ((char)pixels[row][col].getRed());
					hold.add(temp); 
					if(hold.size() > 3) {
	...
	public void hidehardStego(ArrayList<Character> bins) {
		Pixel[][] pixels = this.getPixels2D();
		String temp = ""; 
		for (int row = 0, hold = 0; row < pixels.length && hold < bins.size(); row++) {
			for (int col = 0; col < pixels[0].length && hold < bins.size(); col++) {
				temp = Integer.toBinaryString(pixels[row][col].getBlue());
				if(!(bins.get(hold).equals(temp.charAt(temp.length() -1)))) {
					temp = temp.substring(0, temp.length() -1) + bins.get(hold); 
					while(temp.length() < 8) {
						temp = "0" + temp; 
					}
					pixels[row][col].setBlue(Integer.parseInt(temp, 2));
					//hold++; 
				}
				hold++; 
			}
		}
		this.write("hardRick.jpg");
	}
	...
	public ArrayList<String> getHardNums() {
		...
		for (int row = 0; row < pixels.length; row++) {
			for (int col = 0; col < pixels[0].length; col++) {
				floop = Integer.toBinaryString(pixels[row][col].getBlue()); 
